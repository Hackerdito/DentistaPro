...
export const PatientView: React.FC<PatientViewProps> = ({ appointmentId, onBack, isPublicView = true }) => {
  ...
  useEffect(() => {
    ...
    // FIX: Use standard imports instead of require() to prevent browser crash
    const docRef = doc(db, 'appointments', appointmentId);
    
    // Subscribe to the specific document for real-time updates (chat, status)
  ...
  return (
    <div className="min-h-screen bg-gray-100 dark:bg-black flex flex-col items-center justify-center py-4 sm:py-8 print:bg-white print:p-0 print:h-auto transition-colors duration-300">
      
      {/* Mobile-like Container */}
      <div className="w-full max-w-md bg-white dark:bg-dark-card min-h-[650px] sm:rounded-[2.5rem] shadow-2xl relative flex flex-col overflow-hidden print:shadow-none print:max-w-none print:w-full print:rounded-none transition-colors duration-300">
        ...
        {/* Header Image/Banner - Dental Specific */}
        <div className="h-56 bg-primary-600 relative overflow-hidden print:h-20 print:bg-white print:text-black shrink-0">
            {/* Dental Office Image */}
            <img 
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent z-10 print:hidden"></div>
            
            <div className="absolute bottom-6 left-6 z-20 text-white print:relative print:text-black print:p-0 print:left-0 w-full pr-6">
                <p className="text-sm font-medium opacity-90 mb-1 print:hidden tracking-wide">Bienvenido,</p>
                <h1 className="text-3xl font-bold truncate tracking-tight shadow-sm">{appointment.patientName}</h1>
                <p className="hidden print:block text-sm text-gray-500">Comprobante de Cita</p>
            </div>
        </div>

        {/* Content */}
        <div className="flex-1 px-6 py-6 -mt-8 relative z-30 rounded-t-[2rem] bg-white dark:bg-dark-card border-t dark:border-t-0 print:mt-0 print:p-0 overflow-y-auto">
            
            {/* Status Badge */}
            <div className="flex justify-between items-center mb-6 print:justify-start">
                ...
                <span className="text-xs text-gray-400 dark:text-gray-500">ID: {appointment.id.slice(0,6)}</span>
            </div>

            {/* Date & Time Card */}
            <div className="grid grid-cols-2 gap-4 mb-6 print:border print:border-gray-200 print:rounded-lg print:p-4">
                <div className="bg-gray-50 dark:bg-gray-800 p-4 rounded-2xl border border-gray-100 dark:border-gray-700 text-center print:bg-white print:border-none print:p-0 print:text-left shadow-sm">
                    ...
                    <p className="text-lg font-bold text-gray-900 dark:text-white">{format(new Date(appointment.date), 'dd MMM yyyy', { locale: es })}</p>
                </div>
                <div className="bg-gray-50 dark:bg-gray-800 p-4 rounded-2xl border border-gray-100 dark:border-gray-700 text-center print:bg-white print:border-none print:p-0 print:text-left shadow-sm">
